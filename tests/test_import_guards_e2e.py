...
Both cases here run a program from ``tests/_utils/guard_programs/`` in a *fresh*
...
state one of these programs has to start out without.
...
The programs are real ``.py`` files rather than source passed to ``python -c``, so
...
PROGRAMS = REPO / "tests" / "_utils" / "guard_programs"


def run(program: str) -> str:
    """Run ``guard_programs/<program>.py`` in a fresh interpreter, returning its last line."""
    proc = subprocess.run(  # noqa: S603 - fixed interpreter, path built from a literal, no shell
        [sys.executable, str(PROGRAMS / f"{program}.py")],
        capture_output=True,
        text=True,
        cwd=REPO,
        check=False,
...
def test_discovery_finds_experimental_impls_only_after_opting_in() -> None:
    ...
    The program walks the gate from closed to open and reports one flag per step;
