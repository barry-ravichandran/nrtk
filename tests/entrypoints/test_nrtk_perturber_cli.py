...
class TestNRTKPerturberCLI:
    ...
    # @mock.patch("nrtk.entrypoints._nrtk_perturber_cli._create_output_dataset", return_value=None)
    @mock.patch(
        "nrtk.entrypoints._nrtk_perturber_cli.from_config_dict",
        return_value=MagicMock(__len__=MagicMock(return_value=4)),
    )
    @mock.patch(
        "nrtk.entrypoints._nrtk_perturber_cli.nrtk_perturber",
        return_value=[
            ("_f-0.012_D-0.001_px-2e-05", make_mock_dataset()),
            ("_f-0.012_D-0.003_px-2e-05", make_mock_dataset()),
            ("_f-0.014_D-0.001_px-2e-05", make_mock_dataset()),
            ("_f-0.014_D-0.003_px-2e-05", make_mock_dataset()),
        ],
    )
    @mock.patch("nrtk.entrypoints._nrtk_perturber_cli.dataset_to_coco", return_value=None)
    def test_nrtk_perturber_combine_dataset(
        self,
        dataset_to_coco_patch: MagicMock,
        entrypoint_patch: MagicMock,
        from_config_dict_patch: MagicMock,  # noqa: ARG002
        # create_output_dataset_patch: MagicMock,
        config_file: Path,
        tmpdir: py.path.local,
    ) -> None:
        """Test that entrypoint and dataset_to_coco are called with appropriate arguments."""
        ...
        dataset_to_coco_patch.assert_called_once()

        kwargs = dataset_to_coco_patch.call_args.kwargs

        assert kwargs["output_dir"] == output_dir
        assert kwargs["img_filenames"] == img_filenames
        assert kwargs["dataset_categories"] == dset_cats

        combined_dataset = kwargs["dataset"]

        assert len(combined_dataset) == sum(len(d) for _, d in entrypoint_patch.return_value)
